...
3. **Dispatch loop — one fresh subagent per pending task**
   ...
   b. **Get the implementer prompt** from the [## Implementer Subagent Prompt](#implementer-subagent-prompt) appendix below.

   c. **Spawn a fresh subagent** with the contents of the implementer prompt, replacing `TASK_FILE_PATH` with the actual task file path.
   ...
   d. **Handle response**:
      ...
      - **Failure (blocker)**: Do NOT mark the task complete. Pause and use the appropriate tool for asking the user a question or requesting input to ask for guidance:
